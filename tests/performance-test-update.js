...
function setup(){
    ...
    for (let i = 0; i <prod_lines; i++) {
        for (let j = 0; j <machines_per_line; j++) {
            ...
            prologEngine.jsToPrologEventEmitter.emit('device_event', {type:"update", id:randomId, creator:"machine_"+i+"_"+j, subject:"machine_"+i+"_"+j, data:{parameter:"alarm", value:"off"}, update_property: "parameter"});
...
function startmonitor(){
    ...
    prologEngine.monitor('line_'+last_prod_line, event => {
        ...
        if(counter< test_amount) {
            ...
            setTimeout(simulateDeviceEvent, 500);
        }
